...
def calculate_risk(
    ...
    if len(first_degree_indices) >= 1:
        risk_scores.append(get_lifetime_risk(
            ONE_FIRST_DEG_TABLE, patient_age, first_degree_indices[0]))

    if len(second_degree_indices) >= 1:
        risk_scores.append(get_lifetime_risk(
            ONE_SECOND_DEG_TABLE, patient_age, second_degree_indices[0]))

    if len(first_degree_indices) >= 2:
        risk_scores.append(get_lifetime_risk(
            TWO_FIRST_DEG_TABLE, patient_age, first_degree_indices[0], first_degree_indices[1]))

    if mother_index is not None:
        if len(maternal_aunt_indices) >= 1:
            risk_scores.append(get_lifetime_risk(
                MOTHER_MATERNAL_AUNT, patient_age, mother_index, maternal_aunt_indices[0]))
        if len(paternal_aunt_indices) >= 1:
            risk_scores.append(get_lifetime_risk(
                MOTHER_PATERNAL_AUNT, patient_age, mother_index, paternal_aunt_indices[0]))

    if len(maternal_second_degree_indices) >= 2:
        risk_scores.append(get_lifetime_risk(TWO_SEC_DEG_SAME_SIDE_TABLE, patient_age,
                                             maternal_second_degree_indices[0], maternal_second_degree_indices[1]))

    if len(paternal_second_degree_indices) >= 2:
        risk_scores.append(get_lifetime_risk(TWO_SEC_DEG_SAME_SIDE_TABLE, patient_age,
                                             paternal_second_degree_indices[0], paternal_second_degree_indices[1]))

    if len(maternal_second_degree_indices) >= 1 and len(paternal_second_degree_indices) >= 1:
        risk_scores.append(get_lifetime_risk(TWO_SEC_DEG_DIFF_SIDE_TABLE, patient_age,
                                             maternal_second_degree_indices[0], paternal_second_degree_indices[0]))
...
def get_lifetime_risk(table, patient_age, relative1_index, relative2_index=None):
    ...
    lifetime_risk = _lookup_claus_table(
        table, LIFETIME_AGE_INDEX, relative1_index, relative2_index)
