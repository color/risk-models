...
def calculate_risk(patient_age,
    ...
    first_degree_ages = sort_ages([
        x for x in
        first_degree_ages + full_sister_onset_ages + daughter_onset_ages if x
    ])

    # Assemble the age categories and do an ascending sort
    second_degree_ages = sort_ages([
        x for x in maternal_aunt_onset_ages + paternal_aunt_onset_ages +
        maternal_grandmother_onset_ages + paternal_grandmother_onset_ages +
        maternal_half_sister_onset_ages + paternal_half_sister_onset_ages if x
    ])

    maternal_second_degree_ages = sort_ages([
        x for x in maternal_aunt_onset_ages + maternal_grandmother_onset_ages +
        maternal_half_sister_onset_ages if x
    ])

    paternal_second_degree_ages = sort_ages([
        x for x in paternal_aunt_onset_ages + paternal_grandmother_onset_ages +
    ...
    for ages, table in ((first_degree_ages, ONE_FIRST_DEG_TABLE),
        ...
        if ages:
            risk = max(risk,
                       get_lifetime_risk(table, patient_age,
    ...
    for ages, table in ((first_degree_ages,
                         TWO_FIRST_DEG_TABLE), (maternal_second_degree_ages,
                                                TWO_SEC_DEG_SAME_SIDE_TABLE),
                        (paternal_second_degree_ages,
                         TWO_SEC_DEG_SAME_SIDE_TABLE)):
        if len(ages) > 1:
            risk = max(risk,
                       get_lifetime_risk(table, patient_age,
...
def get_lifetime_risk(table,
    ...
    """
    lifetime_risk = _lookup_claus_table(table, -1, relative1_index,
                                        relative2_index)
    ...
    current_age_risk = _lookup_claus_table(table, patient_age_lower_bin_index,
                                           relative1_index, relative2_index)

    if patient_age_over_bin:
        patient_age_upper_bin_risk = _lookup_claus_table(
            table, patient_age_lower_bin_index + 1, relative1_index,
            relative2_index)

        current_age_risk += (patient_age_upper_bin_risk - current_age_risk
                             ) * patient_age_over_bin / 10

    return round((lifetime_risk - current_age_risk) / (1 - current_age_risk),
                 3)


def _lookup_claus_table(table,
                        patient_index,
                        relative1_index,
                        relative2_index=None):
